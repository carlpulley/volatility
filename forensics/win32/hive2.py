...
class HiveAddressSpace:
    def __init__(self, baseAddressSpace, types, hive_addr):
        self.base = baseAddressSpace
        self.types = types
        self.hive = hive_addr
        
        self.baseblock = read_obj(self.base, self.types, ['_HHIVE', 'BaseBlock'],
            self.hive)
        self.flat = read_obj(self.base, self.types, ['_HHIVE', 'Flat'],
            self.hive) > 0
    ...
    def save(self, outf):
        ...
        length = read_obj(self.base, self.types, ['_HHIVE', 'Storage', 0, 'Length'],
                          self.hive)
    ...
    def stats(self, stable=True):
        ...
        length = read_obj(self.base, self.types, ['_HHIVE', 'Storage', stor, 'Length'],
                          self.hive)
