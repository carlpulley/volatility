...
def get_secrets(sysaddr, secaddr, profile):
    ...
    if not bootkey:
        print "Error: Unable to read the bootkey from the SYSTEM hive, cannot continue"
        return
    lsakey = get_lsa_key(secaddr, bootkey, profile)
    if not lsakey:
        print "Error: Unable to read the LSA key from the SECURITY hive, cannot continue"
        return None
