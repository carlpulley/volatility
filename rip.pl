...
xpsp2types.update(regtypes)
from regwrap import RegWrapper
END
py_bind_class("Parse::Win32Registry", "__main__", "RegWrapper", "get_root_key", "get_timestamp", "get_timestamp_as_string");
...
# correct
	...
	foreach my $i (sort {$a <=> $b} keys %plugins) {
		eval {
			require "rrplugins/".$plugins{$i}."\.pl";
