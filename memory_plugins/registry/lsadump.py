...
class lsadump(forensics.commands.command):
    ...
    def execute(self):
        ...
        syshiveobj = Object("_CMHIVE", self.opts.syshive, addr_space, profile=prof)
        sechiveobj = Object("_CMHIVE", self.opts.sechive, addr_space, profile=prof)
        try:
            if not syshiveobj.Hive.BaseBlock.FileName.endswith("SYSTEM"):
                print "WARN: hive at %x does not appear to be a SYSTEM hive, but continuing anyways" % syshiveobj.offset
            if not sechiveobj.Hive.BaseBlock.FileName.endswith("SECURITY"):
                print "WARN: hive at %x does not appear to be a SECURITY hive, but continuing anyways" % sechiveobj.offset
        except:
            pass

        secrets = get_memory_secrets(addr_space, types, self.opts.syshive, self.opts.sechive, prof)
        if not secrets:
